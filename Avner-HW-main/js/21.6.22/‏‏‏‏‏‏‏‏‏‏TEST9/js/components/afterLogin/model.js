...
export async function getUpdateUsersList(role) {
  var requestOptions = {
    method: 'GET',
  };

  return await fetch(`https://mira-webschool-80ba75.appdrag.site/api/auth/getUpdateUsersList?role=${role}`, requestOptions)
    .then(response => response.json())
    .then(result => { return result })
    .catch(error => { return error });
}

export async function editUser(_id, _uName, _fName, _lName,  _role, _pass){
 
var formdata = new FormData();
formdata.append("uName",  _uName);
formdata.append("fName", _fName);
formdata.append("lName", _lName);
formdata.append("pass", _pass);
...
return await fetch("https://mira-webschool-80ba75.appdrag.site/api/auth/editUser", requestOptions)
.then(response => response.json())
    .then(result => { return result })
    .catch(error => { return error });
